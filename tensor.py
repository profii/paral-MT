...
# TENSOR_PARALLEL_USE_NATIVE=True CUDA_VISIBLE_DEVICES=2,3 torchrun --standalone --nproc_per_node=2 tensor.py
# You split each layer's weights into parts, multiply each part on a separate GPU, then gather results.
...
TENSOR_PARALLEL_USE_NATIVE=os.environ.get("TENSOR_PARALLEL_USE_NATIVE")
print('TENSOR_PARALLEL_USE_NATIVE:', TENSOR_PARALLEL_USE_NATIVE)
tensor_parallel_config = get_default_config(model, device_ids=device_ids)
model = TensorParallel(model, device_ids=[local_rank], output_device=local_rank,
                       distributed=True, tensor_parallel_config=tensor_parallel_config,
                       sharded=True)
## Distributed
...
print('BATCH', BATCH/WORLD_SIZE)
print('N_EPOCHS', N_EPOCHS)
print(f"#{local_rank}| Number of parameters: {total_params}")

if is_wandb:
    wandb.run.name = "tensor_"+str(BATCH)+"b_"+str(N_EPOCHS)+"ep_AdamW_"+str(local_rank)
...
path = "best_tensor"
path_best = "best_tensor/model_"+str(N_EPOCHS)+"ep_"+str(BATCH)+"b.pth"
...
print('destroy_process_group, Device #', device)

# Load best model:
model = Transformer(src_vocab, trg_vocab, embed_dim, nhead, num_encoder_layers, dropout, num_decoder_layers, dim_feedforward).to(device)
model.load_state_dict(torch.load(path_best, map_location=torch.device(device)))

test_loss, bleu = evaluate(model, test_iter, criterion, 1)
